...
public class FriendshipService extends SplitWiseService {
  ...
  public static final String RECEIVED_FRIENDSHIP_NOTIFICATION =
      "%s added you as a friend. " + START_SPLITTING;
  public static final String ADDED_TO_GROUP_NOTIFICATION =
      "You have been added to group %s with participants: %s ." + START_SPLITTING;
  ...
  public void createFriendship(String addingUsername, String addedUsername)
  ...
  public void createGroupFriendship(
      String groupCreator, String groupName, List<String> membersUsernames)
      throws FriendshipException {
    List<User> members =
        membersUsernames.stream()
            .map(username -> userRepository.getById(username).get())
            .collect(Collectors.toList());

    members.stream()
        .forEach(member -> addGroupMember(groupCreator, member, groupName, membersUsernames));
    saveChanges();
  }

  private void addGroupMember(
      String groupCreator, User memberToAdd, String groupName, List<String> members) {
    memberToAdd.addToGroup(groupName, members);
    String addedToGroupNotification =
        String.format(
