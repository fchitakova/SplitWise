...
public class CommandParser {
  ...
  private static boolean hasEqualCountOfParametersAndArguments(
      String[] commandArguments, String commandFormat) {
  ...
  public static boolean matchPayedCommand(String command) {
    ...
    boolean commandMatchesPayedCommand =
        startsWith(command, PAYED_COMMAND)
            && hasMinimumNumberOfArguments(
                commandArguments, MINIMUM_COUNT_OF_PAYED_COMMAND_ARGUMENTS);
    boolean commandMatchesPayedInGroupCommand =
        startsWith(command, PAYED_IN_GROUP_COMMAND)
            && hasMinimumNumberOfArguments(
                commandArguments, MINIMUM_COUNT_OF_PAYED_IN_GROUP_COMMAND_ARGUMENTS);
  ...
  public static boolean matchCreateGroupCommandFormat(String command) {
    String[] commandArguments = getArguments(command);
    boolean doesInputMatchCommand =
        hasMinimumNumberOfArguments(
                commandArguments, MINIMUM_COUNT_OF_CREATE_GROUP_COMMAND_ARGUMENTS)
            && startsWith(command, CREATE_GROUP_COMMAND);
    return doesInputMatchCommand;
  }

  private static boolean hasMinimumNumberOfArguments(
      String[] commandArguments, int minimumNumberOfParameters) {
  ...
  public static boolean matchSplitCommand(String command) {
    boolean hasSplitCommandPrefix =
        startsWith(command, SPLIT_COMMAND) || startsWith(command, SPLIT_GROUP_COMMAND);
    if (hasSplitCommandPrefix) {

      String[] commandArguments = getArguments(command);
      String moneyAmountArgument = commandArguments[0];

      return hasMinimumNumberOfArguments(commandArguments, MINIMUM_COUNT_OF_SPLIT_COMMAND_ARGUMENTS)
          && argumentIsValidDouble(moneyAmountArgument);
    }
    return false;
  }
