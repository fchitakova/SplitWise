...
public class SplitWiseApplication {
  ...
  public static void main(String[] args) {
    ...
    if (resolveSplitWiseDependencies(dbFilePath)) {
      SplitWiseServer splitWiseServer =
          new SplitWiseServer(serverSocket, activeUsers, commandFactory);
      splitWiseServer.start();
    } else {
      LOGGER.info(
          "SplitWise application cannot be started because of missing dependencies.See error.log file for more information.");
    }
  }
  ...
  private static boolean createCommandFactory() {
    ...
    commandFactory =
        new CommandFactory(
            new AuthenticationService(userRepository, activeUsers),
            new FriendshipService(userRepository, activeUsers),
            new MoneySplitService(userRepository, activeUsers));
