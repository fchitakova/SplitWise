...
public class SplitWiseServer {
  ...
  public SplitWiseServer(
      ServerSocket serverSocket, ActiveUsers activeUsers, CommandFactory commandFactory) {
  ...
  private void acceptClientConnections() {
    try {
      ...
      if (isRunning()) {
        LOGGER.info(
            "Error during establishing client connection. See error.log for more information.");
        LOGGER.error(
            "Error during establishing client connection." + "Reason: " + e.getMessage(), e);
      }
    }
  }
