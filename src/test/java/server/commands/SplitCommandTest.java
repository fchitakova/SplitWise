...
public class SplitCommandTest {
  private static final String SPLIT_WITH_FRIEND_COMMAND =
      "split 5 " + TEST_USERNAME2 + " limes and oranges";
  private static final String SPLIT_WITH_GROUP_COMMAND =
      "split-group 5 " + GROUP_NAME + " limes and oranges";
  ...
  public void testThatSplittingWithoutBeingLoggedInReturnsLoginOrRegisterMessage() {
    ...
    String assertMessage =
        "Splitting without being logged in must return login or register response";
  ...
  public void testThatSplittingWithNonFriendUserReturnsNotAllowedResponse() {
    ...
    when(moneySplitService.areFriends(TEST_USERNAME1, TEST_USERNAME2)).thenReturn(false);
    splitCommand = new SplitCommand(SPLIT_WITH_FRIEND_COMMAND, moneySplitService);

    String assertMessage =
        "Splitting with non friend user must return response indicating it is not allowed";
    String commandResult = splitCommand.execute();
  ...
  public void testThatSplittingInGroupInWhichInvokerIsNotMemberReturnsNotAllowedResponse() {
    when(moneySplitService.getCurrentSessionsUsername()).thenReturn(TEST_USERNAME1);
    when(moneySplitService.isGroupMember(TEST_USERNAME1, GROUP_NAME)).thenReturn(false);
    splitCommand = new SplitCommand(SPLIT_WITH_GROUP_COMMAND, moneySplitService);

    String assertMessage =
        "Splitting with group of which the invoker is not member must return response indicating it is not allowed";
    String commandResult = splitCommand.execute();
